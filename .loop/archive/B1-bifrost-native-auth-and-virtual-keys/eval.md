...
| Synced admin-creds copy exists in the infra layer | `deployments/infrastructure/secrets.tf` + `services.tf` `nomad_job.prometheus` | A `data "vault_kv_secret_v2"` read of `default/bifrost/credentials` and a `vault_kv_secret_v2` writing `default/prometheus/bifrost-admin` with `username`/`password`; `nomad_job.prometheus` passes a `bifrost_admin_secret` var | deterministic: grep secrets.tf for the data-source read + the write path/fields; grep services.tf prometheus job for the var | 100% |
