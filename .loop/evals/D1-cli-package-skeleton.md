...
exercises the new hooks. No login, no commands, no TUI, no breakglass.
...
| --- | --- | --- | --- | --- |
| The console script exists and runs | `cd cli && uv run localstack --version` and `uv run localstack --help` | Both exit 0. `--version` prints a version; `--help` lists the app. Proves the entrypoint is wired through `[project.scripts]`, not just that files exist | deterministic check (both commands exit 0 with non-empty output) | 100% |
...
| **Dependencies are declared, not just installed** | Read `cli/pyproject.toml`; confirm `cli/uv.lock` is committed | `typer` under runtime deps; `pytest`, `mypy`, `ruff` under dev deps; `requires-python = ">=3.12"`. `cli/uv.lock` is tracked by git. Installing into an environment without recording the dependency is what `uv add` exists to prevent | deterministic check (deps present in pyproject; uv.lock tracked) | 100% |
