...
| **`vault grants` survives both policy shapes** | Fixture tests against a six-block policy AND the three-block shape | Both render without error and resolve the job-scoped paths. **Corrected 2026-07-31: F9 is now applied, so live Vault serves three `path` blocks, not six.** The row stands anyway and is no longer about an imminent switchover: code that hardcodes a block count breaks the next time the policy is narrowed or widened, and this policy has already changed shape once. Keep both fixtures | deterministic check (both fixtures render correctly) | 100% |
...
being scored and cannot be carried over. Re-signed the same day, after the rewrite.
