...
run here. Row 10 makes that boundary checkable rather than aspirational,
because the natural drift of a status panel is toward a dashboard.

**The trap.** A TUI is judged by how it looks when things are fine, and used
when they are not. Rows 5-8 cover the states a developer actually opens it
during, and row 4 covers the one that quietly reports healthy jobs as broken.
...
| A 403 is its own state | Launch with a token that authenticates but lacks `list-jobs` | The node and job widgets render a permission-denied state naming what was denied, not an empty table. Verified live: a brokered `nomad/creds/deploy` token gets 403 on `/v1/jobs` (`nomad_deploy_role.tf:4-5` withholds `list-jobs`), so this is the expected state under today's credentials, not a hypothetical | deterministic check (403 rendered as permission-denied, not empty) | 100% |
