...
| Shims are not installed without asking | `localstack deps` with no flags | No shim is written. Per the operator's provisional Q1 answer: shadowing `nomad` is a change to the developer's machine and is opt-in, with the devcontainer passing `--with-shims` at build time | deterministic check (no shim written by the bare command) | 100% |
