...
Definition of Done: Vault's Nomad secrets engine mints a short-lived Nomad ACL token scoped strictly narrower than the `developer` policy, replacing the deployer's static Nomad bootstrap token.
...
| Guardrail: out-of-scope actions are denied to the minted token | With the minted token: `nomad alloc exec …`, `nomad node status`, `nomad operator raft list-peers` | Each returns permission denied (HTTP 403), proving the policy is narrower than `developer` | Deterministic check | 100% |
