...
group is refused. The pre-existing unmanaged `developer` policy has exactly
one owner afterwards.
...
| **The `developer` policy has exactly one owner** | `grep -rn 'developer' deployments/**/*.tf` and `nomad acl policy list` | Q1's answer is carried out and visible. If imported: the policy is in Terraform state and its live rules are unchanged. If replaced: the original is **deleted**, not left beside the new one. Two policies granting `alloc-exec` with one unmanaged is the failure this row catches, and it is the state the cluster is in today | deterministic check (single owner; no unmanaged duplicate) | 100% |
