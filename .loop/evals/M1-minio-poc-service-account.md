...
| Reviewer judges the multi-IdP shape leaves room for the Vault-as-IdP sibling | `mc admin config get <alias> identity_openid` after subticket 6 | Two named `identity_openid` configs coexist (the `nomad` config plus a placeholder second named config), confirming the multi-IdP shape is real rather than assumed | model + rubric (adversarial review agent) | 4/5 |
