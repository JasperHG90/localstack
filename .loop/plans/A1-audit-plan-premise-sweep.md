...
- Reviews cost roughly 90-150k tokens and 7-16 minutes each. Thirteen deep
  reviews is not the shape; one pattern sweep is.
...
- **Renaming `.localstack` hostnames. T3 owns that sweep** and already
  ...
  same files.
- Deep adversarial review of any single plan. A1 decides WHICH plans need
  that; it does not perform it.
...
2. Sweep the six with the most dependents first — F2, M1, R3, S1, F7, F8 — so
   a defect that propagates is found early.
3. Sweep the remaining seven: L1, L2, M2, R1, R2, R4, F9.
