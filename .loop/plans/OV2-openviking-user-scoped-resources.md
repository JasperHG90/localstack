...
- **R4.** `vault_kv_secret_v2.openviking_user_keys`
  ...
  Honoring this EMPTIES Hermes's memory: `deployments/applications/services/hermes.hcl:244`
...
The acceptance layer above these gates is `.loop/evals/OV2-openviking-user-scoped-resources.md`:
nine scenarios, of which only rows 5 and 8 are deterministic. Rows 1-4, 6, 7
and 9 are scored by a person against the live cluster, so a green gate run does
not satisfy them.
...
- **Hermes, both ways.** Its key is a copy under its own KV prefix
  ...
  else uses, and the failure is silent. Honor R4 and Hermes starts with an
