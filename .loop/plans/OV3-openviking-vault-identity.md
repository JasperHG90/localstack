...
depends_on = []
priority = 100
premise = { Q1 = "The runtime chain is proven by a probe script added to the code surface (widen-surface), not by a labelled proxy. Chosen over declared-proxy because OV1 took the proxy route and left its P22 unverified across two tickets.", Q4 = "Vault's identity issuer is set to the edge base URL https://vault.lab.orangecluster.nl, giving the effective issuer https://vault.lab.orangecluster.nl/v1/identity/oidc.", Q5 = "The operator entity is renamed to jasper. Entity names equal OpenViking account ids and no third identity is carried.", Q6 = "The ovx client half is out of scope. This ticket is verified by the probe script, not by a CLI login." }
measured_against = { Q4 = { kind = "package", version = "1.18.3", ref = "vault/identity_store_oidc.go", note = "getOIDCConfig appends /v1/<ns>/identity/oidc to the configured issuer and the config write rejects an issuer carrying a path." } }
summary = """
...
Q1 settled as `widen-surface`, which adds a runtime probe script to the surface
(R9). That is the only scope added after review; it does not change the L.
...
- **`ovx` and the dashboard.** Settled at Q6: the client half is out of scope.
  This ticket is verified with the probe script (R9), never with a CLI login.
  Teaching `ovx` the Vault login and the mint belongs to a follow-up, in another
  repo.
- **A runtime gate in CI.** Q1 is settled as `widen-surface`, so this ticket
  DOES add a runtime probe (R9). It is operator-run, like
  `scripts/bifrost_smoke.py`, and is deliberately NOT wired into pre-commit or
  CI: no repo gate reaches a deployed service, and adding one is out of scope.
...
**R9. A runtime probe that proves the chain, settled at Q1 (`widen-surface`).**
A script beside `scripts/bifrost_smoke.py`, the repo's precedent for probing a
deployed service. It mints a token against the R2 role and asserts BOTH
directions, because a grant alone cannot distinguish a working mapping from one
that accepts everything:

- **Grant.** A token for an entity whose name matches an OpenViking account is
  accepted on a data route, and resolves to that account.
- **Denial.** A token whose entity name matches no account, and a token carrying
  a foreign audience, are both refused.

It is operator-run and not wired into any gate (see §5). This is the producer
for §6's headline requirement and for the eval rows that score it; without it
that requirement has no observable, which is the defect Q1 exists to close.
...
| `deployments/infrastructure/oidc.tf` | after `:47` | Q4 settled as (b): a `vault_identity_oidc` resource setting the issuer to `https://vault.lab.orangecluster.nl`, with NO path — Vault rejects a path outright and appends `/v1/identity/oidc` itself. |
| `scripts/ov_identity_probe.py` | new file | R9's runtime probe: mint a token against the R2 role, assert the grant, the unknown-entity denial and the foreign-audience denial. Operator-run, no hook. Working name; match `scripts/bifrost_smoke.py`'s shape. |
...
**The acceptance layer is `.loop/evals/OV3-openviking-vault-identity.md`.** It
carries eleven scored rows and is the Definition of Done above these gates. Two
rows are deterministic and covered by `just pre_commit` and the CLI suite; the
rest need a deployed service or a local container and are operator-run, which is
the reason R9 exists. Its sign-off is DELEGATED, not operator-reviewed, and the
marker says so in its own header.
...
**SETTLED: `widen-surface`.** Decided by the coordinating agent under an explicit
grant of loop control from JasperHG90, 2026-09-07, operator offline. R9 adds the
probe script and §7 carries it; §2 and §5 are revised to match. OV1 took
`declared-proxy` for the same wall and the result was that its P22 went two
tickets unverified, so the labelled-proxy route is refused here.
...
is needed at all. Against the LIVE server it cannot be probed without flipping the mode (P10):
the api-key path always verifies existence and root is refused on data routes.
It CAN be probed before the flip, off the live server: upstream's own denial
message names trusted mode, which asserts an arbitrary account behind a root-key
compare with no existence check, so a local run of the pinned image settles it.
P10 and P15 carry that.
...
**SETTLED: (b), the edge base URL.** Decided by the coordinating agent under an
explicit grant of loop control from JasperHG90, 2026-09-07, operator offline.
Terraform writes `https://vault.lab.orangecluster.nl` — no path, which Vault
rejects — and `ov.conf.json` carries
`https://vault.lab.orangecluster.nl/v1/identity/oidc`. P14 demonstrates that
string from Vault's source rather than inferring it. Still verify it at
implementation time by minting one token and decoding `iss` before writing it
into the config.
...
**SETTLED: rename.** Decided by the coordinating agent under an explicit grant of
loop control from JasperHG90, 2026-09-07, operator offline. `operator` becomes
`jasper`; no third identity is carried. Costs one entity recreate (§9 risk 1)
and the `login.py:76` default, both already in §7.
...
**SETTLED: no, out of scope.** Decided by the coordinating agent under an
explicit grant of loop control from JasperHG90, 2026-09-07, operator offline.
§5 now names it. R9's probe is what proves the chain here; `ovx` is a follow-up
in another repo.
