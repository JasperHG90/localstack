...
premise = { Q1 = "OpenViking v0.4.17.1 rejects an s3 AGFS backend that omits access_key or secret_key, and its Rust S3 client constructs static credentials with session_token=None, so MinIO STS credentials cannot be expressed at all; session_token is also a forbidden extra key on S3Config and backend is restricted to local|s3|memory, so no config route reaches the SDK default chain", Q2 = "OpenViking's openai and cohere rerank clients each build their own flat body and both send documents as a list of plain strings, which Bifrost 1.6.7 rejects with a flat 400 before the request reaches embark; the litellm client hands the same list to a third-party library whose wire output was not measured, and the vikingdb client does wrap each document as an object but signs for Volcengine's own host and cannot be aimed at Bifrost. Bifrost 2.0.0 accepts a bare string in RerankDocument.UnmarshalJSON, which removes the incompatibility, so the operator resolved this fork by upgrading the gateway rather than bypassing it", Q3 = "Vault's lab OIDC provider advertises only the authorization_code grant, so no headless client can mint an ID token for it and oauth2-proxy's cookie_refresh cannot work against it", Q4 = "the openviking database already carries an openviking schema whose ov_collections and ov_indexes hold zero rows and already match ov-postgres v0.2.0's DDL column for column, with its three helper functions installed, plus five dbg_fts* schemas from operator experimentation", Q5 = "Bifrost's ollama provider is Ollama Cloud, and a requested model must exist in Bifrost's catalog for that provider or the call 403s regardless of the virtual key's allowlist", Q7 = "this repo runs three pytest suites in pre-commit, all scoped to cli/tests and the two service backends, so none covers deployments/ or scripts/ and no assertion about a running OpenViking has a producer in the declared code surface" }
measured_against = { Q1 = { kind = "source", version = "OpenViking v0.4.17.1 (git tag v0.4.17.1, image index sha256:14553ec16f2bda9bd08a188cffb659fcfff4fde5891cbb881ed1bd8488b23294)", ref = "openviking_cli/utils/config/agfs_config.py lines 89-108 (S3Config extra=forbid and validate_config) and lines 561-571 (the AGFSConfig dispatch that calls it); crates/ragfs/src/plugins/s3fs/client.rs lines 472-476", note = "the Python validator runs first, so the Rust default-credential-chain branch at line 473 is unreachable from config; ranges differ from main @ 0c5147c, where the dispatch sits at 475-477" }, Q2 = { kind = "probe", version = "OpenViking v0.4.17.1 source, live Bifrost 1.6.7 on radxa, Bifrost transports/v2.0.0 source", ref = "openviking/models/rerank/openai_rerank.py lines 75-102, cohere_rerank.py lines 64-70, volcengine_rerank.py lines 107 and 122; POST /v1/rerank against 192.168.2.50 port 8080; core/schemas/rerank.go lines 26-41 at tag transports/v2.0.0 against the same file at transports/v1.6.7", note = "object documents 200, string documents 400 Invalid request payload with empty routing_info on 1.6.7; transports/v2.0.0 adds RerankDocument.UnmarshalJSON accepting a bare string, which transports/v1.6.7 has not" }, Q3 = { kind = "probe", version = "Vault lab provider discovery document fetched 2026-09-05", ref = "grant_types_supported is exactly [authorization_code]; response_types_supported is [code]; scopes_supported is [email, groups, openid]" }, Q4 = { kind = "probe", version = "PostgreSQL 18.3, pgvector 0.8.2, read 2026-09-05", ref = "schemas dbg_fts, dbg_fts2..dbg_fts5, openviking, public; ov_collections(name,table_name,meta,created_at) and ov_indexes(collection,index_name,meta,created_at), both zero rows; functions ov_array_to_text, ov_path_matches, ov_sparse_dot present", note = "the column shapes match ov-postgres v0.2.0 src/ov_postgres/ddl.py, so the registry is already bootstrapped" }, Q5 = { kind = "repo-scan", version = "working tree at plan time", ref = "deployments/applications/services/bifrost.hcl lines 136-148 and deployments/applications/services.tf lines 609-620" }, Q7 = { kind = "repo-scan", version = "working tree at plan time", ref = ".pre-commit-config.yaml lines 93, 125 and 154 run pytest, scoped to cli/tests and the dash and registry-ui backends; nothing covers deployments/ or scripts/" } }
---
...
authenticate?** The operator asked for MinIO through WI. P1 shows the
...
**Recommendation: (a).** It ships, it is honest about what it is, and it
is a one-line change to (b) later. It is also the shape the operator's
own extension repo assumes: `openviking_extensions` ships a MinIO policy
document naming the `openviking` bucket for a static read-write user.
...
**Q3 — how do headless LAN agents authenticate?** `auth_mode` selects
...
**Recommendation: (a).** Option (d) is listed because it is real and
because it answers the admin question too, not because it is preferred.

**Q4 — what happens to the pre-existing schemas and tables?** Narrower
...
**Recommendation: leave the `openviking` schema in place and let
ov-postgres adopt it**, and drop the `dbg_fts*` schemas by hand in a
separate, operator-run step so no Terraform apply can destroy data. This
ticket touches neither. What is left for the operator is only whether the
`dbg_fts*` schemas go, and when.

**Q5 — which VLM model, and is VLM enabled at all?** Bifrost's `ollama`
...
no model. **Recommendation: leave `vlm` unset in the first deployment**
and add it in a follow-up once a catalogued vision model is chosen;
OpenViking's VLM config validates only when any of its fields is set.

**Q6 — image name and registry.** embark's derived image goes to
`ghcr.io/jasperhg90/embark-jetson`
(`deployments/applications/services.tf:345`), while model weights go to
the cluster registry.
**Recommendation: `ghcr.io/jasperhg90/openviking:<upstream>-<rev>`**,
matching embark, because nodes already authenticate to ghcr.io
...
producer in the declared code surface.** This repo's only gates are
...
**Recommendation: `widen-surface` for the static half plus
`declared-proxy` for the runtime half**, with the proxy's blind spots
named in the eval row. Retargeting R15 to `terraform validate` without
that label is the silent downgrade this fork exists to prevent.
