...
Copied down from `cli/src/localstack_cli/api/health.py` (Requirement 3,
this app's backend must not depend on `cli` at all), in full -- `judge_all`
calls the other three functions here, so there is no narrower subset to
take.

Pure functions over `nomad_client.Job`, so the rule table is a unit test
rather than a snapshot. This is the part of the panel most able to lie, and
a panel that reds a healthy cluster gets ignored and then misses the real
outage.
