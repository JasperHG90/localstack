...
job "postgres" {
  ...
  group "postgres" {
    ...
    task "postgres" {
      ...
      template {
        data = <<EOF
          POSTGRES_USER="{{ with secret "secret/data/default/postgres/config" }}{{ .Data.data.username }}{{ end }}"
          POSTGRES_PASSWORD="{{ with secret "secret/data/default/postgres/config" }}{{ .Data.data.password }}{{ end }}"
        EOF
